...
import lars.view.event.{Sys_time => Event}
  ...
  override def create_elements {
    ...
    checkbox.addListener(new Property.ValueChangeListener() {
      def valueChange(event: Property.ValueChangeEvent) {
        if(checkbox.getValue().asInstanceOf[Boolean]) {
          ...
          Applocal.broadcaster.publish(new lars.view.event.Sys_time('use_os_time))
        } else {
    ...
    button.addListener(new Button.ClickListener() { 
      def buttonClick(event: Button#ClickEvent) {
        ...
        Applocal.broadcaster.publish(new lars.view.event.Sys_time('set_time, timestamp))
      }
    })
